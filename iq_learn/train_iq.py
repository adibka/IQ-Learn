...
def main(args):
    ...
    expert_memory_replay.load((f'experts/{args.env.demo}'),
    ...
    ts_str += f"_d{args.expert.demos}_s{args.seed}"
    log_dir = os.path.join(args.log_dir, args.env.name, ts_str)
...
if __name__ == "__main__":
    ...
    with open(f"conf/{args.env}.yaml", "r") as file:
        config = edict(yaml.load(file, Loader=yaml.FullLoader))

    config.expert.demos = args.demos
    config.agent.init_temp = args.init_temp
